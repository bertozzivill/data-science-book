...
Start off by reading this paper on the motivations and structure behind the ggplot library. It should give you an excellent background for how to think about a `ggplot` object, as well as introductions into common terms. 
...
Now that you know some theory, let's put it into practice! This page is designed to give you some basics of plotting in `ggplot2`. Whenever you print a plot, it will show up in the `Plots` tab in the panel to the bottom right of your RStudio session.

A quick note about terminology: Wickham described `ggplot` as "a layered grammar", but to avoid confusion I think "modular" might be a better term. As you'll notice when we're constructing a plot below, "layer" actually has a specific meaning in `ggplot`: it's an instance of a single `geom` function (that is, something that actually draws data onto the screen in the form of some shape). The other plot components, like labels and faceting and coordinates, are *added on* in the same way `geoms` are, but they are *not* referred to as "layers". For clarity, I'll refer to any modularized plot component as a "component", and only to layers specifically as "layers".

`ggplot` functions are unlike anything else you'll see in R, because they work by *adding* functions to each other. You literally stack things on top of each other, using the `+` sign to add a new component. Get used to this grammar!
...
```{r, include=F}
png(paste0(out_dir, "base_plot.png"))
print(base_plot)
graphics.off()
```
<br> 
...
```{r, include=F}
png(paste0(out_dir, "base_plot_2.png"))
print(base_plot)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "layer_1_2.png"))
print(layer_1)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "layer_1_3.png"))
print(layer_1)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "layer_2.png"))
print(layer_2)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "layer_2_2.png"))
print(layer_2)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "faceting.png"))
print(faceting)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "faceting_2.png"))
print(faceting)
graphics.off()
```
<br>
...
```
```{r, include=F}
png(paste0(out_dir, "faceting_3.png"))
print(faceting)
graphics.off()
```
...
## Here I want a bar plot with a `y` compoent that I specify: the mean salary, by rank.  
...
```
```{r, include=F}
png(paste0(out_dir, "bar_1.png"))
print(bar_1)
graphics.off()

png(paste0(out_dir, "bar_2.png"))
print(bar_2)
graphics.off()
```
...
```{r, include=F}
png(paste0(out_dir, "line_scatter.png"))
print(line_scatter)
graphics.off()
```
<br>
...
```{r, include=F}
png(paste0(out_dir, "density.png"))
print(density)
graphics.off()
```
