...
You calculate the *mean* by adding all these values together and dividing by the number of samples. So here, the mean would be $(1+1+1+1+2+2+5)/5=1.86$. The median, on the other hand, is whatever value is in the middle of this sorted list-- here, it's 1. The benefit of the median is that 50% of the samples will be greater than (or equal to) it, and the other 50% will be less than (or equal to) it. Another word for the median is the **50th percentile** of the data. 
...
**todo: what does it mean to "run a model"?**
